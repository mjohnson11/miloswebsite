...
<h2>And booleans, both long, short, and even grouped:</h2>
<p>bool.js:</p>
<pre><code class="language-javascript">#!/usr/bin/env node
var argv = require('yargs').argv;
...
<h2>And non-hyphenated options too! Just use <code>argv._</code>!</h2>
<p>nonopt.js:</p>
<pre><code class="language-javascript">#!/usr/bin/env node
var argv = require('yargs').argv;
console.log('(%d,%d)', argv.x, argv.y);
...
<h2>Yargs even counts your booleans!</h2>
<p>count.js:</p>
<pre><code class="language-javascript">#!/usr/bin/env node
...
<h2>After yer demands have been met, demand more! Ask for non-hyphenated arguments!</h2>
<p>demand_count.js:</p>
<pre><code class="language-javascript">#!/usr/bin/env node
var argv = require('yargs')
    .demandCommand(2)
...
<h2>EVEN MORE SHIVER ME TIMBERS!</h2>
...
<h2>And if you really want to get all descriptive about it...</h2>
...
<h2>Yargs is here to help you...</h2>
...
<h2><a name="alias"></a>.alias(key, alias)</h2>
...
<h2><a name="array"></a>.array(key)</h2>
...
<h2><a name="boolean"></a>.boolean(key)</h2>
...
exit.</p>
<h2><a name="choices"></a>.choices(key, choices)</h2>
...
<h2><a name="coerce"></a>.coerce(key, fn)</h2>
...
<p>You can also map the same function to several keys at one time. Just pass an
...
<pre><code class="language-js">// --user.name Batman --user.password 123
...
<h2>.command(cmd, desc, [builder], [handler])</h2>
...
<h4>Positional Argument Aliases</h4>
...
<pre><code class="language-js">yargs.command('get &lt;username|email&gt; [password]', 'fetch a user by username or email.')
  .help()
  .argv
</code></pre>
...
<p>Alternatively, a command module may specify an <code>aliases</code> property, which may be
...
<h3>Providing a Command Module</h3>
...
<h2>.commandDir(directory, [opts])</h2>
...
<li>
<p><code>include</code>: RegExp or function</p>
...
<li>
<p><code>exclude</code>: RegExp or function</p>
...
<h2>.completion([cmd], [description], [fn])</h2>
...
<h2><a name="config"></a>.config([key], [description], [parseFn])</h2>
<h2>.config(object)</h2>
...
<h2><a name="conflicts"></a>.conflicts(x, y)</h2>
...
<h2><a name="count"></a>.count(key)</h2>
...
<h2><a name="default"></a>.default(key, value, [description])</h2>
...
<h2><a name="demand"></a>.demand(count, [max], [msg]) [DEPRECATED]</h2>
...
<h2><a name="demandOption"></a>.demandOption(key, [msg | boolean])</h2>
...
<pre><code class="language-javascript">// demand an array of keys to be provided
require('yargs')
  .option('run', {
    alias: 'r',
...
<pre><code class="language-javascript">// demand individual options within the option constructor
require('yargs')
  .options({
    'run': {
      alias: 'r',
...
<h2><a name="demandCommand"></a>.demandCommand(min, [minMsg])</h2>
<h2>.demandCommand(min, [max], [minMsg], [maxMsg])</h2>
...
<pre><code class="language-javascript">require('yargs')
  .command({
    command: 'configure &lt;key&gt; [value]',
    aliases: ['config', 'cfg'],
    desc: 'Set a config variable',
...
expected value.</em></p>
<h2><a name="describe"></a>.describe(key, desc)</h2>
...
<h2><a name="exitprocess"></a>.exitProcess(enable)</h2>
<p>By default, yargs exits the process when the user passes a help flag, uses the
...
<h2><a name="global"></a>.global(globals)</h2>
...
<h2><a name="group"></a>.group(key(s), groupName)</h2>
...
<h2><a name="nargs"></a>.nargs(key, count)</h2>
...
<pre><code class="language-js">const parser = yargs
...
<pre><code class="language-js">// providing the `fn` argument to `parse()` runs yargs in headless mode, this
...
<p><em><strong>Note:</strong></em> Providing a callback to <code>parse()</code> disables the <a href="#exitprocess"><code>exitProcess</code> setting</a> until after the callback is invoked.</p>
...
<h2><a name="requiresArg"></a>.requiresArg(key)</h2>
...
<h2><a name="skipValidation"></a>.skipValidation(key)</h2>
...
<h2><a name="string"></a>.string(key)</h2>
...
<h2><a name="version"></a>.version([option], [description], [version])</h2>
...
<h2><a name="wrap"></a>.wrap(columns)</h2>
