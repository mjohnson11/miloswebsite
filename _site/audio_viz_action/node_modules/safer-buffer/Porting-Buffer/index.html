<h1>Porting to the Buffer.from/Buffer.alloc API</h1>
<p><a id="overview"></a></p>
<h2>Overview</h2>
<ul>
...
<h3>Finding problematic bits of code using grep</h3>
...
exceptions).</p>
<h3>Finding problematic bits of code using Node.js 8</h3>
...
<pre><code class="language-console">$ export NODE_OPTIONS='--trace-warnings --pending-deprecation'
$ cat example.js
'use strict';
...
<h3>Finding problematic bits of code using linters</h3>
...
<h2>Variant 1: Drop support for Node.js ≤ 4.4.x and 5.0.0 — 5.9.x.</h2>
...
<h2>Variant 2: Use a polyfill</h2>
...
<h2>Variant 3 — manual detection, with safeguards</h2>
...
<h3>Buffer(notNumber)</h3>
<p>Before:</p>
<pre><code class="language-js">var buf = new Buffer(notNumber, encoding);
</code></pre>
<p>After:</p>
<pre><code class="language-js">var buf;
...
<pre><code class="language-js">const buf = Buffer.alloc ? Buffer.alloc(number) : new Buffer(number).fill(0);
</code></pre>
<h2>Regarding Buffer.allocUnsafe</h2>
<p>Be extra cautious when using <code>Buffer.allocUnsafe</code>:</p>
<ul>
<li>Don't use it if you don't have a good reason to
<ul>
...
<li>If you use it, make sure that you never return the buffer in a partially-filled state,
<ul>
...
<h3>What is wrong with the <code>Buffer</code> constructor?</h3>
...
<pre><code>function stringToBase64(req, res) {
...
<li>If <code>req.body.string</code> is the <em>number</em> <code>50</code>, the <code>rawBytes</code> would be 50 bytes:
<ul>
...
<h3>The <code>Buffer()</code> constructor has been deprecated for a while. Is this really an issue?</h3>
